...
- NO AI WRITING TELLS: Never use em-dashes or en-dashes anywhere in the resume. Use commas, periods, or hyphens instead. Use only straight quotes and apostrophes (never curly/smart quotes), and no ellipsis characters. Avoid clichéd AI filler ("delve", "leverage" as filler, "seamless", "robust", "moreover", "furthermore"). The output must read as if written by a human executive, not generated.
- PRIORITIZED SUBSTITUTION: Treat this as a fixed-length document you are upgrading in place, not lengthening. Rank existing content by relevance to the target role and spend changes replacing the least job-relevant wording with the most job-relevant wording (missing keywords, required skills, the role's terminology). Every change is a swap of existing words for stronger, more targeted ones — never a net addition. Prioritize the highest-impact areas first (summary, most recent role, core competencies).
- LENGTH / PAGE COUNT: Preserve the resume's original length and page count exactly. Do not make the resume longer. Each "After" must be the same length or shorter than its "Before" (same or fewer characters, and never more lines). Achieve alignment by swapping in stronger, job-specific wording and tightening existing phrasing — not by adding words, bullets, or lines. Do not add brand-new bullets or paragraphs. If a target keyword must go in, offset it by cutting filler in the same bullet so the line count does not increase. A two-page resume must stay two pages.`;
