...
- LENGTH IS A HARD CONSTRAINT (critical): The system AUTOMATICALLY REJECTS and does NOT apply any change whose "after" has more characters than its "before". So to make your edit actually take effect, every "after" MUST have the same number of characters as its "before" or FEWER. Count the characters. Work the target keywords in by REPLACING weaker words and cutting filler (adjectives, "responsible for", "successfully", "in order to", etc.) — a swap, not an addition — so the character count stays equal or drops. Never make a bullet longer. This keeps the resume at its original page count. Do NOT propose additions with an empty "before".
