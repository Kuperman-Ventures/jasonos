...
function sourceSectionBody(item: PinNote): string {
  const parts: string[] = [];
  const body = item.body.trim();
  if (body) parts.push(body);
  if (item.url?.trim()) parts.push(item.url.trim());
  if (item.assetUrl?.trim() && item.assetUrl !== item.url) {
    const label = item.mimeType?.includes("pdf")
      ? "PDF"
      : item.mimeType?.startsWith("image/")
        ? "Image"
        : "File";
    parts.push(`${label}: ${item.assetUrl.trim()}`);
  }
  return parts.join("\n\n") || "(no text)";
}

/**
 * Merge source notes into a kept target. Target keeps id/kind/assets/url;
 * source bodies and extra links are appended; sources are removed.
 */
export function combinePinNotes(
  items: PinNote[],
  targetId: string,
  sourceIds: string[],
): PinNote[] {
  ...
  const sections = sources.map((source) => {
    const heading = source.title.trim() || "Untitled";
    return `## ${heading}\n\n${sourceSectionBody(source)}`;
  });

  const baseBody = target.body.trim();
  const combinedBody = [baseBody, ...sections].filter(Boolean).join("\n\n---\n\n");

  const reviewers = [...new Set([...target.reviewers, ...sources.flatMap((s) => s.reviewers)])];
  const reviewedBy = [...new Set([...target.reviewedBy, ...sources.flatMap((s) => s.reviewedBy)])].filter(
    (id) => reviewers.includes(id),
  );

  const dues = [target.reviewDue, ...sources.map((s) => s.reviewDue)].filter(
    (d): d is string => Boolean(d),
  );
  dues.sort();
  const reviewDue = dues[0] ?? null;

  const drop = new Set(sources.map((s) => s.id));
  return items
    ...
    .map((item) =>
      item.id === targetId
        ? {
            ...
            previewSummary:
              item.kind === "website"
                ? item.previewSummary ?? (combinedBody.slice(0, 600) || null)
                : item.previewSummary,
          }
        : item,
    );
